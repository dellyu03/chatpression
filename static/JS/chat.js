...
function updateConversationCounter() {
  ...
  if (botStatusEl && userData) {
    if (conversationCount < MIN_CONVERSATIONS) {
      // 10회 미만: 최소 대화까지 남은 횟수 표시
      const needed = MIN_CONVERSATIONS - conversationCount;
      botStatusEl.textContent = `분석까지 ${needed}회 남음`;
    } else if (conversationCount < MAX_CONVERSATIONS) {
      // 10~14회: 분석 가능 상태 표시
      const remaining = MAX_CONVERSATIONS - conversationCount;
      botStatusEl.textContent = `분석 가능 · ${remaining}회 남음`;
    } else {
      // 15회: 대화 완료
      botStatusEl.textContent = `대화 완료`;
    }
  }
}
...
function handleEndChat() {
  if (conversationCount < MIN_CONVERSATIONS) {
    const needed = MIN_CONVERSATIONS - conversationCount;
    showSystemNotice(
      `분석을 위해 ${needed}번 더 대화해 주세요. (최소 ${MIN_CONVERSATIONS}회 필요)`,
    );
    return;
  }
  goToAnalyzePage();
